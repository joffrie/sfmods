...
class AntiMentionMod(loader.Module):
    ...
    async def antimentioncmd(self, message):
        ...
        if not message.is_private:
            ...
            am.append(chatid)
            ac.update({chatid: {"action": "none", "exceptions": []}})
            self.db.set("AntiMention", "action", ac)
            self.db.set("AntiMention", "am", am)
            return await message.edit("<b>[AntiMention]</b> Активирован в этом чате.")
        else:
            return await message.edit("<b>[AntiMention]</b> Это не чат!")
    ...
    async def amexcmd(self, message):
        ...
        if not message.is_private:
            ...
            if chatid not in am:
                return await message.edit("<b>[AntiMention]</b> В этом чате режим деактивирован.")
            ...
            if not args and not reply:
                ...
                for _ in ls:
                    ...
                    users += f"• {user.first_name} | <b>[</b><code>{_}</code><b>]</b>\n"
            ...
            if chatid in am:
                if args:
                    if args == "kick":
                        ...
                        return await message.edit("<b>[AntiMention - Settings]</b> Такого режима нет в списке.\nДоступные режимы: kick/ban/mute/none.")
                    self.db.set("AntiMention", "action", ac)
                    ...
                    return await message.edit(f"<b>[AntiMention - Settings] </b> Настройки чата:\n\n:"
                                              f"<b>Состояние режима:</b> True\n"
                                              f"<b>При упоминании в чате будет выполняться действие:</b> {ac[chatid]['action']}\n")
            else:
                return await message.edit("<b>[AntiMention - Settings]</b> В этом чате режим деактивирован.")
        else:
            return await message.edit("<b>[AntiMention]</b> Это не чат!")
